...
@section('content')
    ...
    <div class="nav">
        ...
        <a href="{{ route('completedAction', auth()->user()->id) }}" class="nav-item">
    ...
    <div class="container">
        ...
        <div class="notification">
            <div class="user-info">
                ...
                <div class="user-info-text">
                    <div class="greeting">مرحبًا، {{ $user->name ?? 'مستخدم' }} 👋</div>
        ...
        <div class="plan-status">
            <div class="plan-header">
                ...
                <div class="days-left">تبقّى {{ $user->progress->days_left ?? 45 }} يوم على نهاية خطتك</div>
            </div>
            <div class="progress-container">
                <div class="progress-text"><span>{{ $user->progress->progress ?? 60 }}%</span> تقدّمك:</div>
                <div class="progress-bar">
                    <div class="progress-bar-fill" style="width: {{ $user->progress->progress ?? 60 }}%"></div>
                </div>
            </div>
        </div>
            ...
            <div class="points-details">
                <div class="points-breakdown">
                    <div class="points-text">
                        {{ $user->progress->points ?? 1350 }} نقطة
                        <span class="target-icon">🎯</span>
                    </div>
                    <div class="points-footer">
                        <div class="next-level">محترف – بعد {{ $user->progress->points_to_next_level ?? 150 }} نقطة</div>
                        <div class="current-level">
                            {{ $user->progress->current_level ?? 'مجتهد' }}
                            <span class="medal-icon">🏅</span>
                        </div>
                    </div>
                </div>
                <div class="top-users">ممتاز! أنت من بين <span>{{ $user->progress->top_users_percent ?? 'أعلى 10%' }}</span> من المستخدمين 👏</div>
            </div>
        </div>
        ...
        <div class="achievements">
            ...
            <div class="achievements-details">
                <div class="achievement-item">
                    <div class="achievement-text">أنهيت {{ $user->progress->lessons_completed ?? 8 }} دروس من أصل 40</div>
                    <div class="achievement-icon">📘</div>
                </div>
                <div class="achievement-item streak">
                    <div class="achievement-text">{{ $user->progress->streak_days ?? 7 }} أيام متتالية من الدراسة</div>
                    <div class="achievement-icon">🔥</div>
                </div>
                <div class="achievement-item questions">
                    <div class="achievement-text">أجبت على {{ $user->progress->questions_answered ?? 200 }} سؤال</div>
                    <div class="achievement-icon">💯</div>
                </div>
                <div class="achievement-item completed">
                    <div class="achievement-text">{{ $user->progress->achievements_completed ?? 4 }} إنجازات مكتملة</div>
                    <div class="achievement-icon">🏅</div>
                </div>
            </div>
        </div>
