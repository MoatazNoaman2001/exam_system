...
<div class="settings-container" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="page-header">
        <nav class="breadcrumb-nav">
            <a href="{{ route('student.home') }}">{{ __('lang.home') }}</a>
...
@section('content')
<link rel="stylesheet" href="{{ asset('css/enhanced-settings.css') }}">
...
<div class="settings-container" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="profile-section">
        <div class="profile-content">
            ...
            <div class="profile-info">
                <h2 class="user-name">{{ auth()->user()->name ?? auth()->user()->username ?? __('lang.user') }}</h2>
                <p class="user-email">{{ auth()->user()->email }}</p>
                <div class="profile-stats">
                    <div class="stat-item">
                        <span class="stat-number">85%</span>
                        <span class="stat-label">{{ __('lang.progress') }}</span>
                    </div>
                    <div class="stat-item">
        ...
        <div class="settings-card app-card">
            ...
            <div class="card-body">
                <div class="settings-item toggle-item">
                    ...
                    <label class="switch">
                        <input type="checkbox" {{ auth()->user()->notifications_enabled ?? true ? 'checked' : '' }} 
                ...
                <div class="settings-item">
                    ...
                    <div class="language-buttons">
                        <a href="{{ route('locale.set', 'ar') }}" 
                           class="btn-lang {{ app()->getLocale() == 'ar' ? 'active' : '' }}">
                            العربية
                        </a>
                        <a href="{{ route('locale.set', 'en') }}" 
                           class="btn-lang {{ app()->getLocale() == 'en' ? 'active' : '' }}">
                            English
                        </a>
                    </div>
                </div>
    ...
    if (notificationToggle) {
        notificationToggle.addEventListener('change', function() {
            ...
            .then(data => {
                if (data.success) {
                    showNotification(
                        enabled ? '{{ __("lang.notifications_enabled") }}' : '{{ __("lang.notifications_disabled") }}', 
                        'success'
                    );
                } else {
                    // Revert toggle on error
