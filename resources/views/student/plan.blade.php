...
@section('content')
    ...
    <div class="container py-4">
        <div class="mobile-plan-setup-custom">
            <form action="{{ route('plan.update') }}" method="POST" @if(!Auth::check()) disabled @endif>
                @csrf

                @if(session('success'))
                    <div id="custom-success" class="custom-success fade-in">
                        ✔ تم تحديث الخطة بنجاح! تبقى {{ $progress->days_left ?? 0 }} يوم على نهاية الخطة.
                    </div>
                @endif
                ...
                <div class="mobile-plan-setup-custom__main-content">
                    ...
                    <div class="card custom-card surface p-3 mt-3" style="display: {{ isset($progress->plan_duration) && $progress->plan_duration == 0 ? 'block' : 'none' }}" id="custom-dates">
                        <div class="d-flex flex-column flex-md-row gap-3">
                            <div class="custom-date">
                                ...
                                <input type="date" name="start_date" value="{{ isset($progress->plan_end_date) ? \Carbon\Carbon::parse($progress->plan_end_date ?? now())->format('Y-m-d') : '' }}" id="start-date">
                    ...
                    <div class="card dashed-card p-3 mt-3">
                        ...
                        <p class="text-primary mb-1" id="weekly-lessons">📘 {{ $weeklyLessons }} دروس أسبوعيًا</p>
                        <p class="text-primary" id="weekly-questions">💡 حل {{ $weeklyQuestions }} سؤال تدريبي في الأسبوع</p>
                    </div>
                </div>

                {{-- زر الحفظ --}}
                <button type="submit" class="button-plan text-white text-center fw-bold w-100 mt-4" @if(!Auth::check()) disabled @endif>حفظ وتحديث الخطة</button>
            </form>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // تحديث عرض التواريخ بناءً على الاختيار
            ...
            // حساب الخطة بناءً على المدة أو التواريخ
            ...
            // الاستماع لتغيير المدة أو التواريخ
            ...
            // حساب الخطة عند التحميل
            calculatePlan();

            // التحكم في الـ Custom Success Message
            if (customSuccess) {
                container.style.opacity = '0.5'; // تبعيض الصفحة
                setTimeout(() => {
                    customSuccess.classList.add('fade-out');
                    setTimeout(() => {
                        customSuccess.style.display = 'none';
                        container.style.opacity = '1'; // رجوع الشفافية للطبيعي
                    }, 500); // زمن الانتقال
                }, 10000); // 10 ثواني
            }
        });
